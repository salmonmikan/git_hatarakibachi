...
- 公開予約画面向けreadは、イベント・予約枠・残数・枠履歴を同一DBスナップショットから返す単一RPCにまとめ、複数リクエスト間の整合性をクライアントで再構築しない。
...
- 公開readはcanonical RPCを1本だけ公開し、availability/historyのような分割read APIを互換目的で残さない。
